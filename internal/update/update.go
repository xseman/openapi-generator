// Package update checks GitHub for a newer openapi-generator and replaces this binary
...
const Repo = "xseman/openapi-generator"
...
// `-X github.com/xseman/openapi-generator/internal/update.Version=1.2.3`.
var Version = Dev
...
func Asset() string {
	name := "openapi-generator-" + runtime.GOOS + "-" + runtime.GOARCH
...
// Release is the newest published openapi-generator: its version, where this platform's
...
// `openapi-generator update` replaces a build that was not made from a tag.
...
func Download(ctx context.Context, rel Release, dir string, progress func(done, total int64)) (string, error) {
	...
	f, err := os.CreateTemp(dir, ".openapi-generator-update-*")
	if err != nil {
		return "", err
	}
...
func install(ctx context.Context, rel Release, dest string, progress func(done, total int64)) error {
	...
	// starting openapi-generator while this runs.
	//
	// Windows locks a running .exe against replacement but not against a
	// rename, so the old binary moves aside first. The .old it leaves is
	// removed by the next update; ponytail: one stale .old stays behind
	// until then, a start-up sweep would clear it sooner.
	if runtime.GOOS == "windows" {
		old := dest + ".old"

		_ = os.Remove(old) // the previous update's, if it is still there
...
func request(ctx context.Context, url string) (*http.Request, error) {
	...
	req.Header.Set("User-Agent", "openapi-generator/"+Version) // the GitHub API rejects requests without one
