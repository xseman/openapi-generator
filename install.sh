#!/bin/sh
# openapi-generator installer.
#
#   curl -fsSL https://raw.githubusercontent.com/xseman/openapi-generator/master/install.sh | sh
#
# OPENAPI_GENERATOR_VERSION pins a release (default: the latest), OPENAPI_GENERATOR_INSTALL_DIR the
# directory the binary lands in (default: ~/.local/bin).
set -eu

REPO="xseman/openapi-generator"
BIN="openapi-generator"
INSTALL_DIR="${OPENAPI_GENERATOR_INSTALL_DIR:-$HOME/.local/bin}"

main() {
	echo ""
	echo "   ⌁ openapi-generator installer"
	...
	case "$(uname -s)" in
		Linux) os="linux" ;;
		Darwin) os="darwin" ;;
		*) err "unsupported OS: $(uname -s) — openapi-generator runs on Linux and macOS" ;;
	esac
	case "$(uname -m)" in
	...
	echo ""
	log "run '${BIN} --help' to get started"
	echo ""
}
